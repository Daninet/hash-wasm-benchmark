...
suite.add(`hash-wasm ${getVersion('hash-wasm')} md5()`, async (buf) => {
  return wasmMD5(buf);
});

suite.add(`hash-wasm ${getVersion('hash-wasm')} createMD5()`, (buf) => {
  md5hasher.init();
  md5hasher.update(buf);
  return md5hasher.digest();
});

suite.add(`spark-md5 ${getVersion('spark-md5')}`, (buf) => {
  return sparkmd5.ArrayBuffer.hash(buf);
});

suite.add(`md5-wasm ${getVersion('md5-wasm')}`, (buf) => {
  return npmMD5Wasm(buf);
});

suite.add(`md5 ${getVersion('md5')}`, (buf) => {
  return npmMD5(buf);
});
